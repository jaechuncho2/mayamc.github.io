...
홈페이지는 각 저널에서 최신 논문을 최대 20편씩 별도로 조회한 뒤 PMID 중복을 제거합니다.
따라서 JAVMA처럼 등록 논문 수가 많은 저널이 전체 목록을 독점하지 않습니다.

변경하려면 `index.html`과 `research.html`의 아래 값을 수정합니다.

```javascript
const RESULTS_PER_JOURNAL = 20;
```

한글 요약 자동화에서도 `.github/workflows/update-summaries.yml`의
`RESULTS_PER_JOURNAL` 값을 사용합니다.
